...
public class FeedbackController {
    ...
    public ResponseEntity<FeedbackDto> create(
        ...
        try {
            ...
            // ✅ Send thank-you email after successful save
            try {
                String userEmail = (authorEmail != null && !authorEmail.isBlank())
                        ? authorEmail
                        : req.getUserEmail();

                String userName = ((req.getUserFirstName() != null ? req.getUserFirstName() : "") + " "
                        + (req.getUserLastName() != null ? req.getUserLastName() : "")).trim();

                String serviceName = req.getService() != null ? req.getService().getName() : "Service";

                feedbackMailService.sendThankYouEmail(userEmail, userName, serviceName, body.comment());
            } catch (Exception mailEx) {
                System.err.println("⚠️ Feedback email send failed: " + mailEx.getMessage());
            }
    ...
    @Transactional(readOnly = true)
    public ResponseEntity<PageResponse<FeedbackDto>> list(
    ...
    @Transactional(readOnly = true)
    public ResponseEntity<PageResponse<FeedbackDto>> listForAgent(
            @RequestHeader(HttpHeaders.AUTHORIZATION) String auth,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size) {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @Transactional
    public ResponseEntity<FeedbackDto> acknowledge(
